...
# Introduction
In this notebook, we will learn how to create our first network in R.
...
# Example on creating a basic network
```{r}
...
example_al <- as_adj_edge_list(example_g)
```
...
Write network as an adjacency matrix
```{r adjacency matrix}
...
```{r export the data}
# save as graphml
write_graph(city_network,"city_network.graphml",format="graphml")

# read the same
city_network_read<- read_graph("city_network.graphml", format ="graphml") # note: it adds an id attribute
```
