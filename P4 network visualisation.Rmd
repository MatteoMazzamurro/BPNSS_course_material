...
Fairly similar visualisation techniques
...
ggmap::register_stadiamaps("72639591-13c1-46a8-b58d-86318dbbe614")
...
edge_on_path <- rep(FALSE, ecount(cibola_i))
edge_on_path[Atsinna_UG494_path] <- TRUE
E(cibola_i)$AU_Path <- edge_on_path
E(cibola_i)$wd <- ifelse(E(cibola_i)$AU_Path,3,1)
...
city_coord_df <- as.data.frame(vertex_attr(city_network_p3)[c("name","lon","lat")])

# Extract edge list from network object
city_edgelist <- data.frame(get.edgelist(city_network_p3)) %>% setNames(c("v1","v2"))
...
ggmap(base_city_map, darken = 0.35) +
  ...
  geom_point(
    data = city_coord_df,
    aes(lon, lat, col = V(city_network_p3)$category, size = V(city_network_p3)$pop),
    alpha = 0.8,
    show.legend = FALSE
  ) +
  geom_text(
...
FIRST ATTEMPT TO CONVERT TO MATRIX LAYOUT

```{r}
ggraph(cibola_i, 'matrix') + 
  geom_edge_point(mirror = TRUE) + 
  #geom_node_text(aes(label = name), size = 3, nudge_x = 0.2, nudge_y = 0.2) +
  theme_minimal() +
  coord_fixed()
```
